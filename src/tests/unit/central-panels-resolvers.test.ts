...
describe('resolveProblemScenario', () => {
  it.each(problemScenarioSources.map((scenario) => [scenario.id]))('resolves %s deterministically', (id) => {
...
describe('resolveProductDirection', () => {
  it.each(productDirectionSources.map((direction) => [direction.id]))('resolves %s deterministically', (id) => {
