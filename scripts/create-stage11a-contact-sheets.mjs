...
async function createSheet({ mobile, outputName }) {
  ...
  for (let index = 0; index < zones.length; index += 1) {
    ...
    const fileName = `stage11a-zone-${mobile ? 'mobile-' : ''}${slug}.png`;
    const sourcePath = path.join(actualDir, fileName);
    if (!fs.existsSync(sourcePath)) throw new Error(`Missing Stage 11A zone screenshot: ${sourcePath}`);
...
const desktop = await createSheet({ mobile: false, outputName: 'stage11a-full-page-desktop-contact-sheet.png' });
const mobile = await createSheet({ mobile: true, outputName: 'stage11a-full-page-mobile-contact-sheet.png' });
